{ config, lib, ... }:
{
  options = {
    impermenence = {
      enable = lib.mkEnableOption "Impermenence" // {
        default = true;
      };
  ...
  config = lib.mkIf config.impermenence.enable (
    ...
    in
    {
      ...
      boot.initrd.postDeviceCommands = lib.mkIf (!config.impermenence.initrdSystemd) (
        lib.mkAfter deleteRoot
      );
      ...
      environment.persistence."/persist/system" = {
        ...
        files = lib.mkIf (!config.impermenence.initrdSystemd) [ "/etc/machine-id" ];
      };
    }
  );
}
